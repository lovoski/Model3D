...
namespace Model3D
{
		...
		double val) const
	{
		...
		cerr << "before: model_small = " << model_small.GetNumOfVerts() << "\t";
		model_small.RemoveShortEdges(0.01);
		cerr << "after: model_small = " << model_small.GetNumOfVerts() << endl;
		CBaseModel model_large(vertList, faceList_large);
		model_large.RemoveUnreferencedVertices();
		cerr << "before: model_large = " << model_large.GetNumOfVerts() << "\t";
		model_large.RemoveShortEdges(0.01);
		cerr << "after: model_large = " << model_large.GetNumOfVerts() << endl;
	...
	void CRichModel::ExtractBoundary()
	{
		...
		int boundaryVertNum = boundaryVerts.size();
		while (!boundaryVerts.empty())
		{
			...
			{
				//bug... two open boundaries touch together...
				cluster.push_back(candidate);
				if (cluster.size() > boundaryVertNum)
				{
					cerr << "cluster.size() > boundaryVertNum" << endl;
					throw exception("cluster.size() > boundaryVertNum");
					return;
				}
	...
	//	PreprocessBaseModelIntoRichModel();
	//}
	//void CRichModel::CompleteModel() 
	//{
	//	vector<int> idFromNew2Old;
	//	map<int, int> idFromOld2New;
	//	for (int i = 0; i < GetNumOfVerts(); ++i)
	//	{
	//		if (isBoundaryVert(i) && !Neigh(i).empty())
	//		{
	//			idFromNew2Old.push_back(i);
	//			idFromOld2New[i] = idFromNew2Old.size() - 1;
	//		}
	//	}
	//	UnionFind uf(idFromNew2Old.size());
	//	for (int i = 0; i < GetNumOfEdges(); ++i)
	//	{
	//		if (IsExtremeEdge(i))
	//		{
	//			int v1 = Edge(i).indexOfLeftVert;
	//			int v2 = Edge(i).indexOfRightVert;
	//			uf.Combine(uf.FindAncestor(idFromOld2New[v1]), uf.FindAncestor(idFromOld2New[v2]));
	//		}
	//	}
	//	uf.UpdateParents2Ancestors();

	//	auto clusters = uf.GetClusters();
	//	for (auto mypair : clusters)
	//	{
	//		int key = mypair.first;
	//		int v_first = idFromNew2Old[key];
	//		vector<int> idFromNew2Old;
	//		map<int, int> idFromOld2New;

	//		vector< CPoint3D> pt3d_list;
	//		int v = v_first;
	//		do
	//		{
	//			idFromNew2Old.push_back(v);
	//			pt3d_list.push_back(Vert(v));
	//			idFromOld2New[v] = idFromNew2Old.size() - 1;
	//			v = Edge(Neigh(v).back().first).indexOfRightVert;
	//		} while (v != v_first);

	//		CPoint3D vecSum(CPoint3D::Origin());
	//		for (int i = 2; i < pt3d_list.size(); ++i)
	//		{
	//			vecSum = vecSum + VectorCross(pt3d_list[0], pt3d_list[1], pt3d_list[i]);
	//		}
	//		vecSum.Normalize();
	//		CPoint3D dir1 = vecSum.GetUnitPerpendicularDir();
	//		CPoint3D dir2 = vecSum * dir1;
	//		vector<Point_2> pt2d_list;
	//		map<Point_2, int> fromPoint2ID;
	//		for (int i = 0; i < pt3d_list.size(); ++i)
	//		{
	//			pt2d_list.push_back(Point_2(pt3d_list[i] ^ dir1, pt3d_list[i] ^ dir2));
	//			fromPoint2ID[pt2d_list.back()] = idFromNew2Old[i];
	//		}
	//		CDT cdt;
	...
	//	PreprocessBaseModelIntoRichModel();
	//}
	//void CRichModel::CompleteModel_SingleComponent()
	//{
	//	bool needFurtherDeletion(true);
	//	do
	//	{
	//		needFurtherDeletion = false;
	//		for (int i = 0; i < GetNumOfEdges(); ++i)
	//		{
	//			if (IsExtremeEdge(i))
	//			{
	...
	//				if (Neigh(v1).size() == 2)
	//				{
	//					//EdgePoint ep(Edge(Neigh(v1)[0].first).indexOfRightEdge, 0.5);
	//					//SplitEdge(ep);
	//					m_UselessFaces.insert(Edge(Neigh(v1)[0].first).indexOfFrontFace);
	//					needFurtherDeletion = true;
	//				}
	//			}
	//		}
	//		if (needFurtherDeletion)
	//		{
	//			decltype(m_Faces) faces_reduced;
	//			for (int i = 0; i < m_Faces.size(); ++i)
	//			{
	//				if (m_UselessFaces.find(i) != m_UselessFaces.end())
	//					continue;
	//				faces_reduced.push_back(Face(i));
	//			}

	//			*this = CRichModel(m_Verts, faces_reduced);
	//		}
	//	} while (needFurtherDeletion);
	//	

	//	UnionFind uf(GetNumOfVerts());
	//	for (int i = 0; i < GetNumOfEdges(); ++i)
	//	{
	//		if (IsExtremeEdge(i))
	//		{
	...
	//		}
	//	}
	//	//this->SaveObjFile("split.obj");
	//	uf.UpdateParents2Ancestors();
	//	auto clusters = uf.GetNontrivialClusters();
	//	vector<CBaseModel::CFace> added;
	//	for (auto mypair : clusters)
	//	{
	//		if (mypair.second.size() <= 2)
	//			continue;
	//		int v_first = mypair.first;

	//		vector< int> pt3d_list;
	//		int v = v_first;
	//		do
	//		{
	//			pt3d_list.push_back(v);
	//			v = Edge(Neigh(v).back().first).indexOfRightVert;
	//		} while (v != v_first);
	...
	//		for (int i = 2; i < pt3d_list.size(); ++i)
	//		{
	//			vecSum = vecSum + VectorCross(Vert(pt3d_list[0]), 
	//				Vert(pt3d_list[i-1]),
	//				Vert(pt3d_list[i]));
	//		}
	//		vecSum.Normalize();
	...
	//		for (int i = 0; i < pt3d_list.size(); ++i)
	//		{
	//			pt2d_list.push_back(Point_2(Vert(pt3d_list[i]) ^ dir1, Vert(pt3d_list[i]) ^ dir2));
	//			fromPoint2ID[pt2d_list.back()] = pt3d_list[i];
	//		}
	//		//cerr << "pt2d_list.size() = " << pt2d_list.size() << endl;
	//		//cerr << "fromPoint2ID.size() = " << fromPoint2ID.size() << endl;
	//		CDT cdt;
	//		cdt.insert_constraint(pt2d_list.begin(), pt2d_list.end(), true);
	//		mark_domains(cdt);
	//		for (CDT::Finite_faces_iterator fit = cdt.finite_faces_begin();
	...
	//				//cerr << "ids : " << id1 << ", " << id2 << ", " << id3 << endl;
	//				//m_Faces.push_back(CFace(id1, id2, id3));
	//				added.push_back(CFace(id1, id2, id3));
	//			}
	//		}
	//	}
	//	decltype(m_Faces) new_faces;
	//	for (int i = 0; i < m_Faces.size(); ++i)
	//	{
	//		if (m_UselessFaces.find(i) != m_UselessFaces.end())
	//			continue;
	//		new_faces.push_back(Face(i));
	//	}
	//	CBaseModel(m_Verts, new_faces).SaveObjFile("before_filling.obj");
	//	for (int i = 0; i < added.size(); ++i)
	//	{
	//		new_faces.push_back(added[i]);
	//	}
	//	CBaseModel(m_Verts, added).SaveObjFile("hole_face.obj");
	//	//CBaseModel::PreprocessVertsAndFacesIntoBaseModel();		
	//	//PreprocessBaseModelIntoRichModel();
	//	auto res = Merge(RepairNonmanifold(m_Verts, new_faces));
	//	*this = CRichModel(res.first, res.second);
	//}
}